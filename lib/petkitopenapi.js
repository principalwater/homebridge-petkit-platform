...
class PetkitOpenAPI {
  ...
  constructor(username, password, countryCode, log) {
    ...
    this.endpoint = (this.countryCode)? new CountryUtil(log).getEndPointWithCountryCode(this.countryCode) : "http://api.petkt.com/latest"
    this.log = log;
  ...
  async getEndpointWithCountryCode(countryCode) {
    try {
      return countryCode ? await new CountryUtil(this.log).getEndPointWithCountryCode(countryCode) : "http://api.petkt.com/latest";
    } catch (error) {
